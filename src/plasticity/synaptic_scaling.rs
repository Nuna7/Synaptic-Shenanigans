...
pub struct SynapticScalingConfig {
    pub target_rate_hz: f32, pub alpha: f32,
    pub w_min: f32, pub w_max: f32,
    pub update_interval_ms: f32, pub rate_window_ms: f32,
    pub enabled: bool,
}

impl Default for SynapticScalingConfig {
    fn default() -> Self {
        Self { target_rate_hz: 5.0, alpha: 1.0, w_min: 0.0, w_max: 20.0,
               update_interval_ms: 500.0, rate_window_ms: 1_000.0, enabled: true }
    }
}
...
impl SynapticScaling {
    pub fn new(n_neurons: usize, config: SynapticScalingConfig) -> Self {
        Self { spike_history: vec![Vec::new(); n_neurons], update_count: 0,
               last_update_time: 0.0, log: Vec::new(), n_neurons, config }
    }

    pub fn record_spike(&mut self, nid: usize, t: f32) {
        if nid < self.n_neurons { self.spike_history[nid].push(t); }
    }

    fn prune(&mut self, t_now: f32) {
        let c = t_now - self.config.rate_window_ms;
        for h in &mut self.spike_history { h.retain(|&t| t >= c); }
    }
    ...
    pub fn scale_weights(&mut self, t_now: f32, post_vec: &[usize], weights: &mut [f32]) -> usize {
        ...
        let scales: Vec<f32> = (0..self.n_neurons).map(|nid| {
            let r = self.estimated_rate(nid);
            if r < 1e-3 { (1.0f32 + alpha * 0.1).min(1.5) } else { (target / r).powf(alpha) }
        }).collect();
        let mut changed = 0;
        for (i, w) in weights.iter_mut().enumerate() {
            ...
            if (s - 1.0).abs() > 1e-4 {
                *w = (*w * s).clamp(self.config.w_min, self.config.w_max);
                changed += 1; self.update_count += 1;
            }
        }
        for (nid, &s) in scales.iter().enumerate() {
            if (s - 1.0).abs() > 0.05 { self.log.push((t_now, nid, s)); }
        }
        changed
    }

    pub fn fraction_at_target(&self) -> f32 {
        if self.n_neurons == 0 { return 0.0; }
