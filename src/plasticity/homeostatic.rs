...
impl Default for HomeostaticConfig {
    fn default() -> Self {
        Self { target_rate_hz: 5.0, tau_h: 10_000.0, theta_min: -70.0,
               theta_max: -40.0, rate_window_ms: 1_000.0,
               update_interval_ms: 100.0, enabled: true }
    }
}

impl HomeostaticConfig {
    pub fn fast() -> Self { Self { tau_h: 500.0, update_interval_ms: 50.0, ..Default::default() } }
}
...
impl HomeostaticState {
    pub fn new(n_neurons: usize, initial_theta: f32, config: HomeostaticConfig) -> Self {
        Self {
            ...
            update_count: 0, last_update_time: 0.0, history: Vec::new(), config,
        }
    }
    ...
    pub fn fraction_at_target(&self) -> f32 {
        ...
        (0..n).filter(|&i| (self.estimated_rate(i) - target).abs() <= 1.0).count() as f32 / n as f32
    }
}

#[derive(Clone, Debug, Default, Serialize, Deserialize)]
pub struct ThresholdStats {
    pub min: f32, pub max: f32, pub mean: f32, pub std: f32, pub n: usize,
}
