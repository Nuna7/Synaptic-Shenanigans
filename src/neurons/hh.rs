...
impl HHPopulation {
    ...
    pub fn heterogeneous(n: usize, base: HHParams, noise_frac: f64, seed: u64) -> Self {
        ...
        let state = HHNeuronState {
            v:            (0..n).map(|_| AtomicCell::new(v_rest + rng.gen_range(-2.0..2.0))).collect(),
            m:            (0..n).map(|_| AtomicCell::new(m0)).collect(),
            h:            (0..n).map(|_| AtomicCell::new(h0)).collect(),
            n:            (0..n).map(|_| AtomicCell::new(n0)).collect(),
            spiked:       (0..n).map(|_| AtomicCell::new(false)).collect(),
    ...
    pub fn snapshot_v(&self) -> Vec<f64> { (0..self.n).map(|i| self.read_v(i)).collect() }
}
...
impl NeuronPopulation for HHPopulation {
    ...
    fn step_range(&self, input_current: &[f32], start: usize) {
        for (local_i, &i_coarse) in input_current.iter().enumerate() {
            ...
            for _ in 0..sub {
                ...
                if !was_above && v >= p.v_spike_thresh { crossed = true; }
            }
    ...
    fn local_spiked(&self, idx: usize) -> bool { self.state.spiked[idx].load() }
    fn read_v(&self, idx: usize) -> f32         { self.state.v[idx].load() as f32 }
...
fn alpha_beta_m(v: f64) -> (f64, f64) {
    let dv = v + 40.0;
    let alpha = if dv.abs() < 1e-7 { 1.0 } else { 0.1*dv / (1.0 - (-dv/10.0).exp()) };
    (alpha, 4.0 * (-(v+65.0)/18.0).exp())
}
fn alpha_beta_h(v: f64) -> (f64, f64) {
    (0.07*(-(v+65.0)/20.0).exp(), 1.0/(1.0+(-(v+35.0)/10.0).exp()))
}
fn alpha_beta_n(v: f64) -> (f64, f64) {
    let dv = v + 55.0;
    let alpha = if dv.abs() < 1e-7 { 0.1 } else { 0.01*dv / (1.0-(- dv/10.0).exp()) };
