...
impl IzhikevichPop {
    ...
    pub fn heterogeneous(spec: &[(NeuronType, f32, f32, f32)]) -> Self {
        ...
        for &(nt, dt, c_override, d_override) in spec {
            ...
            a_v.push(a); b_v.push(b); c_v.push(c_override);
            d_v.push(d_override); dt_v.push(dt); nt_v.push(nt);
        }

        let v_init: Vec<AtomicCell<f32>> = c_v.iter().map(|&c| AtomicCell::new(c)).collect();
        let u_init: Vec<AtomicCell<f32>> = b_v.iter().zip(c_v.iter())
            .map(|(&b, &c)| AtomicCell::new(b * c)).collect();

        Self {
            spike_thresh: (0..n).map(|_| AtomicCell::new(30.0)).collect(),
            a: a_v, b: b_v, c: c_v, d: d_v, dt: dt_v,
            v: v_init, u: u_init,
    ...
    pub fn mixed_cortical(n_exc: usize, n_inh: usize, dt: f32, seed: u64) -> Self {
        ...
        for _ in 0..n_exc {
            ...
            spec.push((NeuronType::RegularSpiking, dt, -65.0 + 15.0*r*r, 8.0 - 6.0*r*r));
        }
        for _ in 0..n_inh {
            let r: f32 = rng.gen_range(0.0..1.0);
            spec.push((NeuronType::Custom { a: 0.02+0.08*r, b: 0.25-0.05*r, c: -65.0, d: 2.0 }, dt, -65.0, 2.0));
        }
        Self::heterogeneous(&spec)
    }
    ...
    pub fn snapshot_v(&self) -> Vec<f32> { (0..self.len()).map(|i| self.read_v(i)).collect() }
}
    ...
    fn step_range(&self, input_current: &[f32], start: usize) {
        ...
        for (local_i, &i_ext) in input_current.iter().enumerate() {
            ...
            for _ in 0..sub_steps {
                ...
                u += dt_sub * a * (b*v - u);
                if v >= 30.0 { fired = true; v = c; u += d; break; }
            }

            self.v[idx].store(v.clamp(-90.0, 35.0));
            self.u[idx].store(u);
            if fired { self.spiked[idx].store(true); }
        }
    }

    fn local_spiked(&self, idx: usize) -> bool { self.spiked[idx].load() }
    fn read_v(&self, idx: usize) -> f32         { self.v[idx].load() }
    fn get_threshold(&self, idx: usize) -> f32  { self.spike_thresh[idx].load() }
    fn set_threshold(&self, idx: usize, v: f32) { self.spike_thresh[idx].store(v); }
