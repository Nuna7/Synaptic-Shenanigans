...
impl AdExProfile {
    pub fn params(self) -> AdExParams {
        ...
        match self {
            Self::AdaptingRS => AdExParams {
                a: 4.0,
                b: 80.0,
                tau_w: 150.0,
                ..base
            },
            Self::Bursting => AdExParams {
                a: -0.5,
                b: 7.0,
                tau_w: 300.0,
                v_r: -47.0,
            ...
            Self::TonicRS => AdExParams {
                a: 0.0,
                b: 0.0,
                tau_w: 100.0,
                ..base
            },
            Self::FastSpiking => AdExParams {
                a: 0.0,
                b: 0.0,
                tau_w: 10.0,
                delta_t: 0.5,
            ...
            Self::TransientBurst => AdExParams {
                a: 4.0,
                b: 200.0,
                tau_w: 500.0,
                ..base
...
pub struct AdExPopulation {
    ...
    pub g_l: Vec<f32>,
    pub e_l: Vec<f32>,
    pub v_t: Vec<AtomicCell<f32>>, // threshold — mutable for homeostasis
    pub delta_t: Vec<f32>,
    pub a: Vec<f32>,
    pub b: Vec<f32>,
    pub tau_w: Vec<f32>,
...
impl AdExPopulation {
    pub fn homogeneous(n: usize, p: AdExParams) -> Self {
        Self {
            ...
            g_l: vec![p.g_l; n],
            e_l: vec![p.e_l; n],
            v_t: (0..n).map(|_| AtomicCell::new(p.v_t)).collect(),
            delta_t: vec![p.delta_t; n],
            a: vec![p.a; n],
            b: vec![p.b; n],
            tau_w: vec![p.tau_w; n],
            v_r: vec![p.v_r; n],
            v_peak: vec![p.v_peak; n],
            dt: vec![p.dt; n],
    ...
    pub fn heterogeneous(n: usize, profile: AdExProfile, noise: f32, seed: u64) -> Self {
        ...
        for i in 0..n {
            ...
            pop.tau_w[i] = (base.tau_w * (1.0 + rng.gen_range(-noise..noise))).max(1.0);
            pop.v[i] = AtomicCell::new(base.e_l + rng.gen_range(-1.0..1.0));
        }
        pop
    }
    ...
    pub fn read_w(&self, idx: usize) -> f32 {
        self.w[idx].load()
    }
    ...
    pub fn snapshot_w(&self) -> Vec<f32> {
        (0..self.n).map(|i| self.read_w(i)).collect()
    }
}
