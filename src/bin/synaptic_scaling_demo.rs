...
fn main() {
    ...
    writeln!(
        csv,
        "t_ms,phase,input_hz,mean_rate_hz,mean_weight,fraction_at_target"
    )
    .unwrap();

    let phases = [
        (1000.0f32, 10.0f32, "Baseline"),
        (1000.0, 50.0, "Perturbation"),
        (2000.0, 50.0, "Scaling"),
    ];
    ...
    for (phase_dur, input_hz, phase_name) in phases {
        ...
        while t_now < phase_end {
            ...
            let mean_w =
                sim.synapses.weight.iter().sum::<f32>() / sim.synapses.weight.len().max(1) as f32;
            let frac = scaler.fraction_at_target();
            println!(
                "{:>8.0}  {:>14}  {:>10.0}  {:>12.2}  {:>12.4}  {:>9.1}%",
                step_end,
                phase_name,
                input_hz,
                rate,
            ...
            writeln!(
                csv,
                "{:.0},{},{},{:.4},{:.4},{:.4}",
                step_end, phase_name, input_hz, rate, mean_w, frac
            )
            .unwrap();
            t_now = step_end;
        }
