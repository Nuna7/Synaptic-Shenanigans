...
fn main() {
    ...
    writeln!(
        csv,
        "t_ms,phase,input_rate_hz,mean_firing_hz,mean_theta_mV,fraction_at_target"
    )
    .unwrap();
    ...
    for (phase_idx, (&phase_dur, &input_rate)) in
        phase_ms.iter().zip(phase_rates.iter()).enumerate()
    {
    ...
    println!(
        ...
        homeo.threshold_stats(),
        homeo.fraction_at_target() * 100.0,
        homeo.update_count,
        sim.spike_log.len()
    );
