...
fn run_topology(name: &'static str, n: usize, syn: Synapse) -> (NetworkMetrics, usize, f32, f64) {
    ...
    let mut t = 0.0f32;
    while t < 500.0 {
        for nid in 0..n { if rng.gen_range(0.0f32..1.0) < 0.1 { sim.push_event(t, nid, 80.0, 0, 0.0); } }
        t += 1.0;
    }
...
fn main() {
    ...
    for (name, syn) in topologies { results.push((name, run_topology(name, n, syn))); }
    println!("\n{:<30} {:>8} {:>8} {:>12} {:>12} {:>12}", "Topology","Synapses","MaxDeg","Spikes","Rate(Hz)","Wall(ms)");
    println!("{}", "-".repeat(90));
    ...
    for (name, (m, spikes, rate, wall)) in &results {
        println!("{:<30} {:>8} {:>8} {:>12} {:>12.1} {:>12.1}", name, m.n_synapses, m.max_degree_out, spikes, rate, wall);
        writeln!(f, "{},{},{},{},{:.4},{:.4}", name, m.n_synapses, m.max_degree_out, spikes, rate, wall).unwrap();
    }
