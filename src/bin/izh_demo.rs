...
fn main() {
    ...
    for nt in &types {
        ...
        for step in 0..steps {
            ...
            if pop.local_spiked(0) {
                spike_times.push(step as f32 * dt);
            }
        }
        ...
        let mean_rate = if mean_isi.is_nan() {
            0.0
        } else {
            1000.0 / mean_isi
        };
        println!(
            "  Spikes: {}   Mean ISI: {:.1} ms   Mean rate: {:.1} Hz",
            spike_times.len(),
            mean_isi,
            mean_rate
        );
    ...
    for _ in 0..1000usize {
        let inputs: Vec<f32> = (0..n)
            .map(|_| {
                if rng.gen_range(0.0..1.0f32) < 0.05 {
                    15.0
                } else {
                    0.0
                }
    ...
    println!(
        "Total spikes: {}   Mean population rate: {:.2} Hz",
        total_spikes,
        total_spikes as f32 / (n as f32 * 0.1)
    );
