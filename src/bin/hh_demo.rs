...
use synaptic_shenanigans::neurons::{HHParams, HHPopulation, NeuronPopulation};
...
fn main() {
    ...
    println!(
        "Steady-state at V = -65.0 mV:  m={:.6}  h={:.6}  n={:.6}",
        m0, h0, n0
    );
    ...
    for &i_level in &[0.0f32, 1.0, 2.0, 5.0, 7.5, 10.0, 15.0, 20.0, 30.0, 50.0] {
        ...
        for step in 0..1000usize {
            lif_sim.push_event(step as f32, 0, i_level * 10.0, 0, 0.0);
        }
        ...
        println!(
            "{:>10.1}  {:>12.1}  {:>12.1}",
            i_level, hh_sp as f32, lif_sp as f32
        );
    ...
    for _ in 0..1000 {
        let i: Vec<f32> = (0..100)
            .map(|i| if i < 50 { i_stim } else { 0.0 })
            .collect();
        het.step_range(&i, 0);
        for n in 0..100 {
            if het.local_spiked(n) {
                total += 1;
            }
        }
    }
    println!(
        "  Total spikes: {}   Mean rate: {:.1} Hz",
        total,
        total as f32 / 100.0
    );
