...
fn run_regime(name: &'static str, n: usize, input_rate: f32, weight: f32, seed: u64) -> (Vec<(f32,usize)>, usize, f32) {
    let ep = EdgeParams { weight, delay: 1.5, inhibitory_fraction: 0.2, inh_weight_scale: 3.0, tau_syn: 5.0, e_inh: -70.0 };
...
fn main() {
    ...
    let regimes = vec![
        ...
        run_regime("Synchronous-Bursting", n, 3.0, 12.0, 43),
        {
            ...
            for nid in 0..n { for t in pat.generate(0.0, dur) { sim.push_event(t, nid, 80.0, 0, 0.0); } }
            sim.run_auto(dur);
            println!("  Oscillatory done ({} spikes)", sim.spike_log.len());
            (sim.spike_log, n, dur)
        },
    ];
    let names = ["Async-Irregular", "Synchronous-Bursting", "Oscillatory(40Hz)"];
    ...
    for ((spikes, n, dur), name) in regimes.iter().zip(names.iter()) {
        ...
        println!("{:<35} spikes={} rate={:.1}Hz χ={:.4} CV={:.3} Fano={:.3} dom={:.1}Hz bursts={}",
            name, spikes.len(), rate, sync.chi, isi.cv, isi.fano_factor, dom, bursts.len());
        writeln!(csv, "{},{},{:.3},{:.4},{:.3},{:.3},{:.1},{}", name, spikes.len(), rate, sync.chi, isi.cv, isi.fano_factor, dom, bursts.len()).unwrap();
    }
