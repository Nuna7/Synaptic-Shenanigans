...
fn main() {
    ...
    let stdp_cfg = StdpConfig { a_plus: 0.01, a_minus: 0.0085, tau_plus: 16.8, tau_minus: 33.7,
                                w_min: 0.0, w_max: 15.0, enabled: true };
    ...
    let n_trials = 50; let trial_dur = 150.0f32; let stim_w = 140.0f32;
    println!("\n{:>6}  {:>12}  {:>12}  {:>12}", "Trial", "Mean W", "Max W", "STDP Updates");

    for trial in 0..n_trials {
        let t_start = trial as f32 * trial_dur;
        for i in 0..n_pre { sim.push_event(t_start + 2.0, i, stim_w, 0, 0.0); }
        sim.run_auto(t_start + trial_dur);

        // KEY CHANGE: reset neurons via trait method reset_neuron — no direct field access
        for i in 0..n_total { sim.neurons.reset_neuron(i, -65.0); }

        let mut trial_spikes: Vec<(f32,usize)> = sim.spike_log.iter()
            .filter(|&&(t,_)| t >= t_start && t < t_start + trial_dur).cloned().collect();
        ...
        for &(t, nid) in &trial_spikes {
            ...
            last_t = t;
            stdp.accumulate_for_spike(nid, t, &sim.synapses.pre, &sim.synapses.post, &sim.pre_index);
        }
        ...
        println!("{:>6}  {:>12.4}  {:>12.4}  {:>12}", trial+1, stats.mean, stats.max, updated);

        for (si, &w) in sim.synapses.weight.iter().enumerate() {
            if si < syn_pre_snap.len() {
                writeln!(csv, "{},{},{},{},{:.6}", trial+1, si, syn_pre_snap[si], syn_post_snap[si], w).unwrap();
            }
        }
    }
    ...
    println!("Change:              {:.4} ({:+.1}%)", final_stats.mean - initial_weight,
