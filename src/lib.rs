...
// Neuron models
pub use neurons::{NeuronPopulation, NeuronPartition};
pub use neurons::{LifNeuron, IzhikevichPop, NeuronType, HHPopulation, HHParams,
                  AdExPopulation, AdExProfile, AdExParams};

// Plasticity
pub use plasticity::{StdpState, StdpConfig, WeightStats,
                     HomeostaticState, HomeostaticConfig, ThresholdStats,
                     SynapticScaling, SynapticScalingConfig,
                     PlasticityRule, PlasticityContext};

// Network topology (old Synapse-returning NetworkBuilder, used by demos/tests)
pub use network::{NetworkBuilder, EdgeParams, NetworkMetrics};

// Input generation
pub use input::poisson::{PoissonSource, PoissonPopulation, StimulusPattern, drive_background};

// Core
pub use simulation::{Simulation, SchedulerMode, SimConfig, replay_equal};
pub use synapse::{Synapse, SynapseMatrix, synapse_current};
pub use metrics::{SynchronyIndex, BurstDetector, Burst, AvalancheResult, ISIStats,
...
#[unsafe(no_mangle)]
pub extern "C" fn neurosim_create(n_neurons: u32, n_threads: u32, seed: u64, scheduler: u32) -> u64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn neurosim_load(path_ptr: *const std::os::raw::c_char) -> u64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn neurosim_step(id: u64, until_time: f32, out_buf: *mut FfiSpike, buf_len: u32) -> u32 {
    with_sim(id, |sim| {
        let spikes = sim.step_until(until_time);
        let n = spikes.len().min(buf_len as usize);
        for (i, &(t, nid)) in spikes.iter().take(n).enumerate() {
            unsafe { *out_buf.add(i) = FfiSpike { time: t, neuron_id: nid as _ }; }
        }
        n as u32
    }).unwrap_or(0)
}
...
pub extern "C" fn neurosim_push(id: u64, time: f32, neuron: u32, weight: f32) {
    with_sim(id, |sim| sim.push_event(time, neuron as usize, weight, 0, 0.0));
}

#[unsafe(no_mangle)]
pub extern "C" fn neurosim_get_voltages(id: u64, out_buf: *mut f32, buf_len: u32) -> u32 {
    with_sim(id, |sim| {
        let v = sim.get_all_voltages();
        let n = v.len().min(buf_len as usize);
        unsafe { std::ptr::copy_nonoverlapping(v.as_ptr(), out_buf, n); }
        n as u32
    }).unwrap_or(0)
}

#[unsafe(no_mangle)] pub extern "C" fn neurosim_spike_count(id: u64) -> u32 {
    with_sim(id, |s| s.spike_count() as u32).unwrap_or(0)
}
#[unsafe(no_mangle)] pub extern "C" fn neurosim_current_time(id: u64) -> f32 {
    with_sim(id, |s| s.current_time()).unwrap_or(0.0)
}
#[unsafe(no_mangle)] pub extern "C" fn neurosim_clear_spikes(id: u64) {
    with_sim(id, |s| s.clear_spikes());
}
#[unsafe(no_mangle)] pub extern "C" fn neurosim_inject_spike(id: u64, neuron: u32, weight: f32) {
    with_sim(id, |s| s.inject_spike(neuron, weight));
}
#[unsafe(no_mangle)] pub extern "C" fn neurosim_free(id: u64) {
    SIMS.lock().unwrap().remove(&id);
}

#[unsafe(no_mangle)]
pub extern "C" fn neurosim_checkpoint(id: u64, path_ptr: *const std::os::raw::c_char) -> u32 {
    let path = unsafe { std::ffi::CStr::from_ptr(path_ptr).to_str().unwrap_or("") };
...
#[repr(C)]
pub struct SimHandle { sim: *mut Sim }

#[unsafe(no_mangle)]
pub unsafe extern "C" fn sim_create_basic(n_neurons: c_int, n_threads: c_int, seed: c_ulong) -> *mut SimHandle {
    let neurons = LifNeuron::new(n_neurons as usize, -65.0, -50.0, 20.0, 1.0, 1.0, 5.0);
...
pub unsafe extern "C" fn sim_free(handle: *mut SimHandle) {
    ...
    unsafe { let h = Box::from_raw(handle); if !h.sim.is_null() { drop(Box::from_raw(h.sim)); } }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn sim_step_and_query(handle: *mut SimHandle, end_time: c_float) -> c_int {
    if handle.is_null() { return -1; }
    unsafe { let sim = &mut *(*handle).sim; sim.scheduler_mode = SchedulerMode::SingleThreaded; sim.run_auto(end_time); }
    0
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn sim_set_scheduler(handle: *mut SimHandle, mode: c_int, n_threads: c_int) -> c_int {
    if handle.is_null() { return -1; }
    unsafe {
...
pub unsafe extern "C" fn sim_get_time(handle: *mut SimHandle, out_t: *mut c_float) -> c_int {
    ...
    unsafe { *out_t = (*(*handle).sim).time; } 0
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn sim_push_current(handle: *mut SimHandle, time: c_float, target: c_int, weight: c_float) -> c_int {
    if handle.is_null() { return -1; }
    if target < 0 { return -2; }
    unsafe {
        let sim = &mut *(*handle).sim;
        if target as usize >= sim.neurons.len() { return -2; }
...
pub unsafe extern "C" fn sim_clear_spikes(handle: *mut SimHandle) -> c_int {
    if handle.is_null() { return -1; }
    unsafe { (*(*handle).sim).spike_log.clear(); } 0
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn sim_get_spikes(handle: *mut SimHandle, out: *mut FfiSpike, max: c_int) -> c_int {
    ...
    unsafe {
        ...
        for (i, (t, nid)) in log.iter().take(n as usize).enumerate() {
            slice[i] = FfiSpike { time: *t, neuron_id: *nid as c_int };
        }
        n
    }
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn sim_get_voltage(handle: *mut SimHandle, neuron: c_int, out_v: *mut c_float) -> c_int {
    if handle.is_null() || out_v.is_null() { return -1; }
    if neuron < 0 { return -2; }
    unsafe {
        let sim = &*(*handle).sim;
        if neuron as usize >= sim.neurons.len() { return -2; }
...
pub unsafe extern "C" fn sim_save_checkpoint(handle: *mut SimHandle, path: *const c_char) -> c_int {
    ...
    let path_str = match unsafe { CStr::from_ptr(path) }.to_str() { Ok(s) => s, Err(_) => return -2 };
    unsafe {
        let sim = &*(*handle).sim;
        if sim.save_state(path_str, &format!("{path_str}.sha256")).is_err() { return -3; }
    }
    0
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn sim_load_checkpoint(path: *const c_char, seed: c_ulong, n_threads: c_int) -> *mut SimHandle {
