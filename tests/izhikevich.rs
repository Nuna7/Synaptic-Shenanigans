...
fn izhikevich_deterministic_single_neuron() {
    ...
    for _ in 0..100 { p1.step_range(&[10.0],0); p2.step_range(&[10.0],0); }
...
fn izhikevich_all_types_fire_at_high_current() {
    use synaptic_shenanigans::neurons::{IzhikevichPop, NeuronType, NeuronPopulation};
    for nt in [NeuronType::RegularSpiking, NeuronType::FastSpiking, NeuronType::Chattering] {
        let pop = IzhikevichPop::homogeneous(1, nt, 1.0);
        let mut spiked = false;
        for _ in 0..500 { pop.step_range(&[15.0],0); if pop.local_spiked(0) { spiked = true; } }
...
fn izhikevich_reset_neuron() {
    ...
    for _ in 0..100 { pop.step_range(&[15.0],0); }
