...
fn hh_no_spike_below_rheobase() {
    assert_eq!(count_spikes(2.0, 500), 0);
}
#[test]
fn hh_fires_above_rheobase() {
    assert!(count_spikes(10.0, 500) > 5);
}
#[test]
fn hh_membrane_potential_stays_in_range() {
    let (_, vs) = run_single(10.0, 500);
    for v in &vs {
        assert!(*v >= -90.0 && *v <= 60.0);
    }
}
#[test]
fn hh_fi_curve_is_monotonically_increasing() {
    let rates: Vec<usize> = [6.0f32, 8.0, 10.0, 15.0, 20.0]
        .iter()
        .map(|&i| count_spikes(i, 1000))
        .collect();
...
fn hh_deterministic_across_runs() {
    ...
    for _ in 0..500 {
        h1.step_range(&[10.0], 0);
        h2.step_range(&[10.0], 0);
