...
fn count_spikes(profile: AdExProfile, i_ext: f32, ms: usize) -> usize {
    ...
    let mut n = 0;
    for _ in 0..ms { pop.step_range(&[i_ext], 0); if pop.local_spiked(0) { n += 1; } }
    n
}

#[test] fn adex_fires_above_threshold()  { assert!(count_spikes(AdExProfile::AdaptingRS, 500.0, 500) > 0); }
#[test] fn adex_silent_at_zero_current() { assert_eq!(count_spikes(AdExProfile::AdaptingRS, 0.0, 500), 0); }
#[test]
fn adex_adapting_rs_rate_decreases() {
    let pop = AdExPopulation::from_profile(1, AdExProfile::AdaptingRS);
    let mut sp = vec![];
    for step in 0..2000usize { pop.step_range(&[600.0], 0); if pop.local_spiked(0) { sp.push(step); } }
    let first  = sp.iter().filter(|&&s| s <  500).count();
    let second = sp.iter().filter(|&&s| s >= 500 && s < 1000).count();
    assert!(second <= first, "first={first} second={second}");
}
#[test]
fn adex_all_profiles_fire_at_high_current() {
    for p in [AdExProfile::AdaptingRS, AdExProfile::Bursting, AdExProfile::TonicRS,
...
fn adex_reset_neuron_via_trait() {
    let pop = AdExPopulation::from_profile(1, AdExProfile::AdaptingRS);
    for _ in 0..200 { pop.step_range(&[600.0], 0); }
...
fn adex_deterministic_across_runs() {
    ...
    for _ in 0..500 {
        p1.step_range(&[500.0],0); p2.step_range(&[500.0],0);
